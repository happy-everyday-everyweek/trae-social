...
- `classifyError` 不依赖 className 匹配作为 SDK 异常门控——直接对任意非网络异常
  反射 `getMethod("statusCode")`，命中即按 HTTP code 分类（401 / 403 / 404 / 429 / 5xx），
  未命中走 message 正则 `\b(4\d{2}|5\d{2})\b` 兜底。网络异常（UnknownHost / timeout / IOException）
  优先于 HTTP code 分类，避免 IOException message 偶然含 3 位数字被误判
  （旧 `getMethod("code")` 是 bug，SDK 暴露的是 `statusCode()`，详见 PR #264 / #271 review）。

## 测试覆盖

- `test/` 下 7 个测试类：`CommentPromptBuilderTest` / `PersonaUpdatePromptBuilderTest` / `PromptUtilsTest` / `TweetPostProcessorTest` / `TweetPromptBuilderTest` / `DefaultRulesetEngineTest`（#151 后：降级链 + JSON mode prompt 降级 + 429 转 `RateLimitedException`，mock `EndpointRegistry`）/ `SdkExceptionClassifierTest`（#308：反射 `statusCode()` 识别 SDK 异常 HTTP 状态码）。
- 覆盖 prompt 解析容错、错别字替换、emoji 追加、截断、相似度校验、降级链与 SDK 异常分类。
- 旧 `ContentFilterTest` 已随 #151 删除 `ContentFilter` 类一并移除（敏感词命中测试不再存在，本地敏感词防线暂缺，详见上文"内容过滤"）。
...
- `EventTextPreParserTest` 现位于 `core-profiling` 模块（`analysis` 子包），#151 后 mock `RulesetEngine`，调用点从旧 `llmRegistry.getClient(provider)` 迁到 `rulesetEngine.chatSync(...)`。
- **#151 follow-up**：`EndpointRegistry` / 两个 `CompatibleClient` / `ConfigRepository` 端点 CRUD + 迁移逻辑仍待补单测，对 +2117/-2081 的重构，降级链 / 429 转换 / 持久性错误判断 / 迁移幂等 / reorder 原子性均应有测试保护（PR #264 review Major 6 已记录）。
