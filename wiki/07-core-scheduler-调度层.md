...
继承 `Service`。`foregroundServiceType = specialUse`（IMPL-17：Android 14+ 避免 `dataSync` 6 小时配额限制）。通知渠道 `"scheduler"` `IMPORTANCE_LOW`，常驻通知"社交生态运行中 / AI 账号正在按计划发推与互动"。
...
fun nextTriggerTime(
    ...
    zone: ZoneId,
    random: Random,
    postsInWindowProvider: (accountId: String, windowStart: Instant, windowEnd: Instant) -> Int,
): Instant?
```
...
- `backoffPolicy = EXPONENTIAL` `BACKOFF_INITIAL_SECONDS = 10` `BACKOFF_MAX_SECONDS = 90`
...
> Note 文档/代码偏差：`SchedulerInitializer` KDoc 声称由 `SocialApp.onCreate` 调用，实际已移至 `MainActivity.onCreate`。
