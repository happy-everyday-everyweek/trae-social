...
/**
 * [EndpointRegistry] 默认实现（#306：从原 `EndpointRegistry` 具体类抽出，类名改为
 * `DefaultEndpointRegistry`，原 `EndpointRegistry` 升级为接口供 DIP 注入）。
 *
 ...
 *   → [DefaultEndpointRegistry] 收集 → [invalidateCache]
 * - 端点 reorder → 同上
 */
@Singleton
class DefaultEndpointRegistry @Inject constructor(
        ...
        scope.launch {
            while (isActive) {
                try {
                    ...
                    Timber.e(e, "DefaultEndpointRegistry 订阅端点变更失败，1s 后重试")
                    try {
                        delay(1000)
                    } catch (e: kotlinx.coroutines.CancellationException) {
                        throw e
     ...
     */
    override suspend fun getClient(endpointId: String): LlmClient? {
        clients[endpointId]?.let { return it }
        return mutex.withLock {
            // Double-check after acquiring lock
            clients[endpointId]?.let { return it }
            val snapshot = configProvider.getEndpoint(endpointId) ?: return null
            ...
            val apiKey = try {
                ...
                Timber.w(e, "DefaultEndpointRegistry.getEndpointApiKey 失败 endpointId=%s", endpointId)
                null
            }
            ...
            if (protocol?.requiresApiKey == true && apiKey.isNullOrBlank()) {
                Timber.w("DefaultEndpointRegistry 端点 API Key 缺失，跳过 client 创建 endpointId=%s protocol=%s", endpointId, protocol.id)
                return null
            }
            val config = EndpointConfig.fromSnapshot(snapshot, apiKey) ?: return null
     ...
     */
    override suspend fun getDefaultClient(): LlmClient? {
        ...
        val endpoints = try {
            ...
            Timber.w(e, "DefaultEndpointRegistry.getDefaultClient: listEndpoints 失败")
            return null
        }
    ...
    override suspend fun listEndpoints(): List<EndpointSnapshot> {
        ...
        return try {
            ...
            Timber.w(e, "DefaultEndpointRegistry.listEndpoints 失败")
            emptyList()
        }
    }
    ...
    override suspend fun getEndpoint(id: String): EndpointSnapshot? {
        ...
        return try {
            ...
            Timber.w(e, "DefaultEndpointRegistry.getEndpoint 失败 id=%s", id)
            null
        }
    }
     ...
     */
    override suspend fun invalidate(endpointId: String) {
