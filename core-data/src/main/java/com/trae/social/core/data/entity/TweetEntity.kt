 ...
 *
 * 索引：createdAt（分页排序）、(authorId, createdAt) 复合索引（按作者查询 + 计数），
 * deduplicationKey（唯一去重）。
...
@Entity(
    ...
    indices = [
        ...
        // #227：移除单列 authorId 索引——复合索引 (authorId, createdAt) 已覆盖 WHERE authorId = ?
        // 的最左前缀查询，单列索引冗余且造成每条 INSERT/UPDATE 多维护一份 B-Tree（写放大）
